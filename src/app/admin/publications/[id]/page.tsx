...
export default function AdminEditPublicationPage() {
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        setDescripcion(j?.descripcion || j?.vehiculo?.descripcion || "");
        setPrecio(j?.precio ?? "");
  ...
  async function deleteImage(img: Img) {
    ...
    try {
      const r = await fetch(`/api/publications/${id}/images/${imageId}`, { method: "DELETE", credentials: "include" });
      if (!r.ok) {
        const j = await r.json().catch(()=>({}));
  ...
  return (
    <main className="fade-in flex justify-center py-12 bg-gray-100 min-h-screen">
      <div className="bg-white shadow-xl rounded-2xl p-6 w-full max-w-3xl">
        <h1 className="text-2xl font-extrabold mb-4">Admin: Editar publicación #{pub.id_publicacion}</h1>
        ...
        <form className="space-y-4" onSubmit={onSubmit}>
          ...
          <div className="flex justify-end gap-2 pt-2">
            <button type="button" className="px-4 py-2 rounded-xl" onClick={()=>router.push("/admin")}>Cancelar</button>
